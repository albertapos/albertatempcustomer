...
// 		$user_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "user WHERE username = '" . $this->db->escape($username) . "' AND (password = SHA1(CONCAT(salt, SHA1(CONCAT(salt, SHA1('" . $this->db->escape($password) . "'))))) OR password = '" . $this->db->escape(md5($password)) . "') AND status = '1'");
		...
		if ($user_query->num_rows && $verify_password) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "user_group SET permission = '" . (isset($new_permission) ? $this->db->escape(json_encode($new_permission)) : '') . "' WHERE user_group_id = '" . 1 . "'");
	...
	public function verifyUserToken($token){
		$query = $this->db->query("SELECT token FROM user_token WHERE token='".$token."'");
		return $query->row['token'];
	}

	public function verifyUserSID($token){
		$query = $this->db->query("SELECT sid FROM user_token WHERE token='".$token."'");
		return $query->row['sid'];
	}

	public function storeDatabase($token){
		$query = $this->db->query("SELECT sid FROM user_token WHERE token='".$token."'");

		$sid = $query->row['sid'];

		$store = $this->db->query("SELECT * FROM stores WHERE id='".$sid."'")->row;

		$db_conn = new \DB('mysqli', $store['db_hostname'], $store['db_username'], $store['db_password'], $store['db_name']);

        return $db_conn;
	}

	public function verifyTokenExpired($token){

		$query = $this->db->query("SELECT * FROM user_token WHERE token='".$token."' AND date_format(date_expired,'%Y-%m-%d') > '".date('Y-m-d')."'")->rows;
