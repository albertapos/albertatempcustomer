...
class ModelApiSalesReport extends Model {
	...
	public function getSalesReport($data) {
		...
		$sql = "SELECT date_format(a.dtrandate,'%m-%d-%Y %H:%i:%s') as dtrandate,vterminalid,concat('TRN',isalesid) as  trnsalesid,isalesid,nnettotal,nchange,ntaxtotal,vtendertype,vtrntype FROM trn_sales a WHERE date_format(a.dtrandate,'%Y-%m-%d') >= '".$data['start_date']."' and date_format(a.dtrandate,'%Y-%m-%d') <= '".$data['end_date']."' ";
