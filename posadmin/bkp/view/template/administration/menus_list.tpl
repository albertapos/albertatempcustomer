...
<div id="content">
  <div class="page-header">
    <div class="container-fluid">
	  ...
	  <a onclick="$('#form-menu').attr('action', '<?php echo $edit_list; ?>'); $('#form-menu').submit();" class="btn btn-primary"><i class="fa fa-save"></i></a>
                ...
                <tr>
                  <td class="text-center"><?php if (in_array($menu['MenuId'], $selected)) { ?>
                   ...
                   <td class="text-center"><?php if ($menu['ImageLoc']) { ?>					
					<a href="" id="thumb-image<?php echo $menu['MenuId']; ?>" data-toggle="image" class="img-thumbnail"><img src="data:image/jpeg; base64, <?php echo $menu['ImageLoc']; ?>" height="30" width="30"/></a>
                    <input type="hidden" name="menu[<?php echo $menu['MenuId']; ?>][image]" value="<?php echo $menu['ImageLoc']; ?>" id="input-image<?php echo $menu['MenuId']; ?>" />					
                    <?php } else { ?>
				  ...
				  <td class="text-right"><a class="btn btn-sm btn-success up" onclick='document.getElementById("menu[<?php echo $menu['MenuId']; ?>][select]").setAttribute("checked","checked");'><i class="fa fa-arrow-up"></i></a> <a class="btn btn-sm btn-warning down" onclick='document.getElementById("menu[<?php echo $menu['MenuId']; ?>][select]").setAttribute("checked","checked");'><i class="fa fa-arrow-down"></i></a> <a href="<?php echo $menu['view']; ?>" data-toggle="tooltip" title="<?php echo $button_view; ?>" class="btn btn-sm btn-info"><i class="fa fa-eye"></i></a></td>
                </tr>
                <?php } ?>
                <?php } else { ?>
                <tr>
