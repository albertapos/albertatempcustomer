		    ...
		    <div class="box">
		        <div class="box-header">
			        ...
			        <div class="text-right" style="padding: 10px 10px 10px 10px;">
			            <a href="/sales/users/create" class="btn btn-success btn-md">
	            		...
	            		<tbody>
		            		@foreach ($users as $user)
				                <tr>
			                        ...
			                        <td><a href="{{ url('sales/users/view',$user->id) }}">{{ $user->fname }} {{ $user->lname }}</a></td>
			                        @if($user->roles)
			                        ...
			                        @endif	
			                        @if($user->office)
			                        	<td>{{ $user->office->title or null}}</td>
			                        @else
			                        	<td> </td>
			                        @endif
			                        <td>{{ $user->phone }}</td>
			                        ...
			                        <td>
			                            <a href="/sales/users/{{ $user->id }}/edit"
...
$('#user_listing').DataTable({
    ...
    'aoColumnDefs': [{
        'bSortable': false,
        'aTargets': [4,5,8] /* 1st one, start by the right */
    }]
...
<script type="text/javascript">
	$(document).on('click', '.del_button', function(event) {
        ...
        $('form#form_user_delete').attr('action','/sales/users/'+user_id);
