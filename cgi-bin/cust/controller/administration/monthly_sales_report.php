...
class ControllerAdministrationMonthlySalesReport extends Controller {
  ...
  public function csv_export() {
    ...
    ini_set("memory_limit", "2G");
        ...
        $file_path = DIR_TEMPLATE."/administration/sales-report.csv";

        $myfile = fopen( DIR_TEMPLATE."/administration/sales-report.csv", "w");
  ...
  public function pdf_save_page() {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
      ...
      $report_datas = $this->model_api_monthly_sales_report->getMonthlyReport($this->request->post);
    ...
    $data['csv_export'] = $this->url->link('administration/monthly_sales_report/csv_export', 'token=' . $this->session->data['token'] . $url, true);
