<?php
class ControllerAdministrationAdjustmentDetail extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('administration/adjustment_detail');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			$this->response->redirect($this->url->link('administration/adjustment_detail', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('administration/adjustment_detail', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('administration/adjustment_detail/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['edit'] = $this->url->link('administration/adjustment_detail/edit', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('administration/adjustment_detail/delete', 'token=' . $this->session->data['token'] . $url, true);
		$data['edit_list'] = $this->url->link('administration/adjustment_detail/edit_list', 'token=' . $this->session->data['token'] . $url, true);

		$data['current_url'] = $this->url->link('administration/adjustment_detail', 'token=' . $this->session->data['token'], true);
		$data['searchadjustment'] = $this->url->link('administration/adjustment_detail/search', 'token=' . $this->session->data['token'], true);
		...
		$adjustment_detail_data = $this->model_api_physical_inventory->getPhysicalInventoriesByType('Adjustment',$filter_data);
		
		$adjustment_detail_total = $this->model_api_physical_inventory->getPhysicalInventoriesByTypeTotal('Adjustment');
		...
		foreach ($results as $result) {
			...
			$data['adjustment_details'][] = array(
				...
				'edit'            => $this->url->link('administration/adjustment_detail/edit', 'token=' . $this->session->data['token'] . '&ipiid=' . $result['ipiid'] . $url, true)
		...
		$pagination->total = $adjustment_detail_total;
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('administration/adjustment_detail', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($adjustment_detail_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($adjustment_detail_total - $this->config->get('config_limit_admin'))) ? $adjustment_detail_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $adjustment_detail_total, ceil($adjustment_detail_total / $this->config->get('config_limit_admin')));

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('administration/adjustment_detail_list', $data));
	}
	...
	protected function getForm() {
		...
		$data['column_adj_qty'] = $this->language->get('column_adj_qty');
		...
		if (!isset($this->request->get['ipiid'])) {
			$data['action'] = $this->url->link('administration/adjustment_detail/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('administration/adjustment_detail/edit', 'token=' . $this->session->data['token'] . '&ipiid=' . $this->request->get['ipiid'] . $url, true);
		}

		$data['cancel'] = $this->url->link('administration/adjustment_detail', 'token=' . $this->session->data['token'] . $url, true);
		$data['add_items'] = $this->url->link('administration/adjustment_detail/add_items', 'token=' . $this->session->data['token'] . $url, true);
		$data['remove_items'] = $this->url->link('administration/adjustment_detail/remove_items', 'token=' . $this->session->data['token'] . $url, true);

		$data['display_items'] = $this->url->link('administration/adjustment_detail/display_items', 'token=' . $this->session->data['token'], true);
		$data['display_items_search'] = $this->url->link('administration/adjustment_detail/display_items_search', 'token=' . $this->session->data['token'], true);
		$data['calculate_post'] = $this->url->link('administration/adjustment_detail/calculate_post', 'token=' . $this->session->data['token'], true);
		$data['adjustment_list'] = $this->url->link('administration/adjustment_detail', 'token=' . $this->session->data['token'], true);

		if (isset($this->request->get['ipiid']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$adjustment_detail_info = $this->model_api_physical_inventory->getPhysicalInventory($this->request->get['ipiid']);
		...
		if (isset($this->request->post['vrefnumber'])) {
			...
			$data['vrefnumber'] = $adjustment_detail_info['vrefnumber'];
		} else {
		...
		if (isset($this->request->post['vordertitle'])) {
			...
			$data['vordertitle'] = $adjustment_detail_info['vordertitle'];
		} else {
			$data['vordertitle'] = '';
		}
		...
		if (isset($this->request->post['dcreatedate'])) {
			...
			$data['dcreatedate'] = $adjustment_detail_info['dcreatedate'];
		} else {
			$data['dcreatedate'] = '';
		}
		...
		if (isset($this->request->post['estatus'])) {
			...
			$data['estatus'] = $adjustment_detail_info['estatus'];
		}

		if (isset($this->request->post['vnotes'])) {
			$data['vnotes'] = $this->request->post['vnotes'];
		} elseif (!empty($adjustment_detail_info)) {
			$data['vnotes'] = $adjustment_detail_info['vnotes'];
		} else {
			$data['vnotes'] = '';
		}

		$this->load->model('administration/adjustment_detail');

		$data['reasons'] = $this->model_administration_adjustment_detail->getReasons();

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('administration/adjustment_detail_form', $data));
	}
	...
	protected function validateEditList() {
  	...
  	public function add_items() {
		...
		if(count($this->request->post['checkbox_itemsort1']) > 0){
			$right_items_arr = $this->model_administration_adjustment_detail->getRightItems($this->request->post['checkbox_itemsort1']);

			// $left_items_arr = $this->model_administration_adjustment_detail->getLeftItems($this->request->post['checkbox_itemsort1']);
	...
	public function remove_items() {
		...
		$left_items_arr = $this->model_administration_adjustment_detail->getLeftItems($data);
		...
		if (isset($this->request->get['ipiid'])) {
			...
			if(isset($adjustment_detail_info)){
				...
				if(isset($adjustment_detail_info['items']) && count($adjustment_detail_info['items']) > 0){
					...
					$itms = $this->model_administration_adjustment_detail->getPrevRightItemIds($adjustment_detail_info['items']);
				}
				
				// $edit_left_items = $this->model_administration_adjustment_detail->getEditLeftItems($itms);

				$edit_right_items =array();
				if(count($itms) > 0){
					$edit_right_items = $this->model_administration_adjustment_detail->getEditRightItems($itms,$this->request->get['ipiid']);
				}
	...
	public function display_items_search() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if(isset($post_arr['search_val']) && isset($post_arr['search_by']) && isset($post_arr['right_items'])){
					...
					$json['items'] = $this->model_administration_adjustment_detail->getSearchItems($post_arr);
			}
		}
