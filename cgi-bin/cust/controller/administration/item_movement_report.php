...
class ControllerAdministrationItemMovementReport extends Controller {
  ...
  public function pdf_save_page() {
    ...
    $data['item_movement_print_data'] = $this->url->link('administration/item_movement_report/item_movement_print_data', 'token=' . $this->session->data['token'], true);
    $data['printpage'] = $this->url->link('administration/item_movement_report/printpage', 'token=' . $this->session->data['token'], true);
  ...
  $sales_header= $this->model_api_item_movement_report->getSalesById($isalesid);
  
  $trn_date = DateTime::createFromFormat('m-d-Y h:i A', $sales_header['trandate']);
  $trn_date = $trn_date->format('m-d-Y');

  $sales_detail= $this->model_api_item_movement_report->getSalesPerview($idettrnid);

  $sales_customer= $this->model_api_item_movement_report->getSalesByCustomer($sales_header['icustomerid']);
  ...
  $myfile = fopen( DIR_TEMPLATE."/administration/print_item_movement_report.tpl", "w");
  fwrite($myfile,$file);
  fclose($myfile);
  ...
  public function printpage(){
    $this->response->setOutput($this->load->view('administration/print_item_movement_report')); 
  }
