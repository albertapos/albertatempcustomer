  ...
  public function csv_export() {
    ...
    ini_set("memory_limit", "2G");
        ...
        if(count($data['reports']) > 0){
            ...
            $data_row .= 'Grand Total,'.$grand_total_qty_sold.',100.00%,$'.number_format((float)$data_grand_total_net_amt, 2, '.', '').',100.00%,$'.number_format((float)$grand_total_total_cost, 2, '.', '').',$'.number_format((float)$grand_total_total_price, 2, '.', '').','.number_format((float)$tot_mark_up, 2, '.', '') .'%'.','.number_format((float)($grand_total_gross_profit / $grand_total_total_price) * 100, 2, '.', '') .'%'.PHP_EOL;
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
      ...
      foreach ($reportsdata as $k => $v) {
        if(array_key_exists($v['vname'],$report_datas)){
            $v['NetAmt'] = $v['TotCostPrice']+$v['TOTUNITPRICE'];
