<?php
class ControllerAdministrationPhysicalInventoryDetail extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('administration/physical_inventory_detail');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			$this->response->redirect($this->url->link('administration/physical_inventory_detail', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('administration/physical_inventory_detail', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('administration/physical_inventory_detail/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['edit'] = $this->url->link('administration/physical_inventory_detail/edit', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('administration/physical_inventory_detail/delete', 'token=' . $this->session->data['token'] . $url, true);
		$data['edit_list'] = $this->url->link('administration/physical_inventory_detail/edit_list', 'token=' . $this->session->data['token'] . $url, true);

		$data['current_url'] = $this->url->link('administration/physical_inventory_detail', 'token=' . $this->session->data['token'], true);
		$data['searchphysical'] = $this->url->link('administration/physical_inventory_detail/search', 'token=' . $this->session->data['token'], true);
		...
		$physical_inventory_detail_data = $this->model_api_physical_inventory->getPhysicalInventoriesByType('Physical',$filter_data);
		
		$physical_inventory_detail_total = $this->model_api_physical_inventory->getPhysicalInventoriesByTypeTotal('Physical');
		...
		foreach ($results as $result) {
			...
			$data['physical_inventory_details'][] = array(
				...
				'edit'            => $this->url->link('administration/physical_inventory_detail/edit', 'token=' . $this->session->data['token'] . '&ipiid=' . $result['ipiid'] . $url, true)
		...
		$pagination->url = $this->url->link('administration/physical_inventory_detail', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('administration/physical_inventory_detail_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['ipiid'])) {
			$data['action'] = $this->url->link('administration/physical_inventory_detail/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('administration/physical_inventory_detail/edit', 'token=' . $this->session->data['token'] . '&ipiid=' . $this->request->get['ipiid'] . $url, true);
		}

		$data['cancel'] = $this->url->link('administration/physical_inventory_detail', 'token=' . $this->session->data['token'] . $url, true);
		$data['add_items'] = $this->url->link('administration/physical_inventory_detail/add_items', 'token=' . $this->session->data['token'] . $url, true);
		$data['remove_items'] = $this->url->link('administration/physical_inventory_detail/remove_items', 'token=' . $this->session->data['token'] . $url, true);

		$data['display_items'] = $this->url->link('administration/physical_inventory_detail/display_items', 'token=' . $this->session->data['token'], true);
		$data['display_items_search'] = $this->url->link('administration/physical_inventory_detail/display_items_search', 'token=' . $this->session->data['token'], true);
		$data['calculate_post'] = $this->url->link('administration/physical_inventory_detail/calculate_post', 'token=' . $this->session->data['token'], true);
		$data['physical_inventory_list'] = $this->url->link('administration/physical_inventory_detail', 'token=' . $this->session->data['token'], true);

		$data['import_physical_inventory'] = $this->url->link('administration/physical_inventory_detail/import_physical_inventory', 'token=' . $this->session->data['token'], true);
		$data['calculate_post_check_data'] = $this->url->link('administration/physical_inventory_detail/calculate_post_check_data', 'token=' . $this->session->data['token'], true);

		$data['add_new_import_physical_inventory'] = $this->url->link('administration/physical_inventory_detail/add_new_import_physical_inventory', 'token=' . $this->session->data['token'], true);

		if (isset($this->request->get['ipiid']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$physical_inventory_detail_info = $this->model_api_physical_inventory->getPhysicalInventory($this->request->get['ipiid']);
		...
		$this->response->setOutput($this->load->view('administration/physical_inventory_detail_form', $data));
	}
	...
	protected function validateEditList() {
  	...
  	public function add_items() {
		...
		if(count($this->request->post['checkbox_itemsort1']) > 0){
			$right_items_arr = $this->model_administration_physical_inventory_detail->getRightItems($this->request->post['checkbox_itemsort1']);
			...
			if(isset($this->request->files['import_physical_inventory_file']) && $this->request->files['import_physical_inventory_file']['name'] != ''){
				...
				$ipiid = $this->request->post['ipiid'];
					...
					if ($handle) {
						...
						$arr = explode(',', $strline);
							if($line_row_index != 1){
								if(count($arr) != 2){
									...
									$item_arr = $this->model_api_physical_inventory->getItemBySKU($item_vbarcode);

									if(count($item_arr) > 0){
										$ipiid_arr = $this->model_api_physical_inventory->getItemBySKUipiid($ipiid,$item_arr['iitemid']);
										if(count($ipiid_arr) == 0){
											...
											$this->model_api_physical_inventory->insertInventory($ipiid,$items);
						...
						$this->model_api_physical_inventory->updatennettotal($ipiid,$item_total_ndebitextprice);
	...
	public function calculate_post_check_data() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			$item_response = $this->model_api_physical_inventory->calclulatePostCheckData($temp_arr);
			$json = $item_response;
		}
