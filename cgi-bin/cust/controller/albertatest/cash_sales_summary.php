<?php
class ControllerAlbertatestCashSalesSummary extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('albertatest/cash_sales_summary');

		$this->document->setTitle($this->language->get('heading_title'));

    $this->load->model('administration/cash_sales_summary');
		$this->load->model('albertatest/cash_sales_summary');
  ...
  public function csv_export() {
      ...
      $file_path = DIR_TEMPLATE."/albertatest/cash-and-sales-summary-report.csv";

      $myfile = fopen( DIR_TEMPLATE."/albertatest/cash-and-sales-summary-report.csv", "w");
  ...
  public function print_page() {
    ...
    $this->response->setOutput($this->load->view('albertatest/print_page_list', $data));
  }
  ...
  public function pdf_save_page() {
    ...
    $html = $this->load->view('albertatest/print_page_list', $data);
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
      if($this->request->post['report_by'] == 1){
        $data['reports'] = $this->model_albertatest_cash_sales_summary->getCategoriesReport($this->request->post);
      }elseif($this->request->post['report_by'] == 2){
        $data['reports'] = $this->model_albertatest_cash_sales_summary->getDepartmentsReport($this->request->post);
      }else{
         $data['reports'] = $this->model_albertatest_cash_sales_summary->getGroupsReport($this->request->post);
      }
      ...
      $this->session->data['desc_title'] = $data['desc_title'];
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('albertatest/cash_sales_summary', 'token=' . $this->session->data['token'] . $url, true)
		);

    $data['reportdata'] = $this->url->link('albertatest/cash_sales_summary/reportdata', 'token=' . $this->session->data['token'] . $url, true);
    $data['print_page'] = $this->url->link('albertatest/cash_sales_summary/print_page', 'token=' . $this->session->data['token'] . $url, true);
    $data['pdf_save_page'] = $this->url->link('albertatest/cash_sales_summary/pdf_save_page', 'token=' . $this->session->data['token'] . $url, true);
    $data['csv_export'] = $this->url->link('albertatest/cash_sales_summary/csv_export', 'token=' . $this->session->data['token'] . $url, true);
    ...
    $data['store_name'] = $this->session->data['storename'];
		...
		$this->response->setOutput($this->load->view('albertatest/cash_sales_summary_list', $data));
	}
