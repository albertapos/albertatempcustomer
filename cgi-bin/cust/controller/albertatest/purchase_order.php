<?php
class ControllerAlbertatestPurchaseOrder extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('albertatest/purchase_order');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('albertatest/purchase_order');
	...
	public function add_post() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			$data_response = $this->model_albertatest_purchase_order->addPurchaseOrder($this->request->post,null,'PO');
	...
	public function edit_post() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			$data_response = $this->model_albertatest_purchase_order->editPurchaseOrder($this->request->post,null,'PO');
	...
	protected function getList() {
		...
		if(isset($this->request->get['order']) && $this->request->get['order'] == 'ASC'){
			$data['sort_estatus'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] .'&sort=estatus&order=DESC&searchbox='.$searchbox, true);
			$data['sort_vponumber'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] .'&sort=vponumber&order=DESC&searchbox='.$searchbox, true);
			$data['sort_vvendorname'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] .'&sort=vvendorname&order=DESC&searchbox='.$searchbox, true);
			$data['sort_vordertype'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] .'&sort=vordertype&order=DESC&searchbox='.$searchbox, true);
			$data['sort_dcreatedate'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] .'&sort=dcreatedate&order=DESC&searchbox='.$searchbox, true);
			$data['sort_dreceiveddate'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] .'&sort=dreceiveddate&order=DESC&searchbox='.$searchbox, true);
			$data['sort_LastUpdate'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] .'&sort=LastUpdate&order=DESC&searchbox='.$searchbox, true);
		}else{
			$data['sort_estatus'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] .'&sort=estatus&order=ASC&searchbox='.$searchbox, true);
			$data['sort_vponumber'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] .'&sort=vponumber&order=ASC&searchbox='.$searchbox, true);
			$data['sort_vvendorname'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] .'&sort=vvendorname&order=ASC&searchbox='.$searchbox, true);
			$data['sort_vordertype'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] .'&sort=vordertype&order=ASC&searchbox='.$searchbox, true);
			$data['sort_dcreatedate'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] .'&sort=dcreatedate&order=ASC&searchbox='.$searchbox, true);
			$data['sort_dreceiveddate'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] .'&sort=dreceiveddate&order=ASC&searchbox='.$searchbox, true);
			$data['sort_LastUpdate'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] .'&sort=LastUpdate&order=ASC&searchbox='.$searchbox, true);
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('albertatest/purchase_order/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['edit'] = $this->url->link('albertatest/purchase_order/edit', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('albertatest/purchase_order/delete', 'token=' . $this->session->data['token'] . $url, true);
		$data['edit_list'] = $this->url->link('albertatest/purchase_order/edit_list', 'token=' . $this->session->data['token'] . $url, true);
		$data['current_url'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
		$data['import_invoice'] = $this->url->link('albertatest/purchase_order/import_invoice', 'token=' . $this->session->data['token'] . $url, true);
		...
		$purchase_order_total = $this->model_albertatest_purchase_order->getTotalPurchaseOrders($filter_data);
		
		$results = $this->model_albertatest_purchase_order->getPurchaseOrders($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['purchase_orders'][] = array(
				...
				'view'          => $this->url->link('albertatest/purchase_order/info', 'token=' . $this->session->data['token'] . '&ipoid=' . $result['ipoid'] . $url, true),
				'edit'          => $this->url->link('albertatest/purchase_order/edit', 'token=' . $this->session->data['token'] . '&ipoid=' . $result['ipoid'] . $url, true),
				'delete'        => $this->url->link('albertatest/purchase_order/delete', 'token=' . $this->session->data['token'] . '&ipoid=' . $result['ipoid'] . $url, true)
			);
		}

		$data['vendors'] = $this->model_albertatest_purchase_order->getVendors();
		...
		$missing_item_results = $this->model_albertatest_purchase_order->getMissingItems();
		...
		$pagination->url = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('albertatest/purchase_order_list', $data));
	}
	...
	protected function validateEditList() {
  	...
  	protected function getForm() {
		...
		if (!isset($this->request->get['ipoid'])) {
			$data['action'] = $this->url->link('albertatest/purchase_order/add_post', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('albertatest/purchase_order/edit_post', 'token=' . $this->session->data['token'] . '&ipoid=' . $this->request->get['ipoid'] . $url, true);
		}

		$data['cancel'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'] . $url, true);

		$data['get_vendor'] = $this->url->link('albertatest/purchase_order/get_vendor', 'token=' . $this->session->data['token'], true);
		$data['get_search_items'] = $this->url->link('albertatest/purchase_order/get_search_items', 'token=' . $this->session->data['token'], true);
		$data['get_search_item'] = $this->url->link('albertatest/purchase_order/get_search_item', 'token=' . $this->session->data['token'], true);
		$data['check_invoice'] = $this->url->link('api/purchase_order/check_invoice', 'token=' . $this->session->data['token'], true);
		$data['purchase_order_list'] = $this->url->link('albertatest/purchase_order', 'token=' . $this->session->data['token'], true);
		...
		$data['save_receive_item'] = $this->url->link('albertatest/purchase_order/save_receive_item', 'token=' . $this->session->data['token'], true);
		$data['add_purchase_order_item'] = $this->url->link('albertatest/purchase_order/add_purchase_order_item', 'token=' . $this->session->data['token'], true);

		$data['get_search_item_history'] = $this->url->link('albertatest/purchase_order/get_search_item_history', 'token=' . $this->session->data['token'], true);
		$data['search_vendor_item_code'] = $this->url->link('albertatest/purchase_order/search_vendor_item_code', 'token=' . $this->session->data['token'], true);
		$data['get_item_history'] = $this->url->link('albertatest/purchase_order/get_item_history', 'token=' . $this->session->data['token'], true);
		$data['get_item_history_date'] = $this->url->link('albertatest/purchase_order/get_item_history_date', 'token=' . $this->session->data['token'], true);
		$data['get_vendor_item'] = $this->url->link('albertatest/purchase_order/get_vendor_item', 'token=' . $this->session->data['token'], true);
		$data['get_vendor_item_code_data'] = $this->url->link('albertatest/purchase_order/get_vendor_item_code_data', 'token=' . $this->session->data['token'], true);

		$data['check_warehouse_invoice'] = $this->url->link('albertatest/transfer/check_invoice', 'token=' . $this->session->data['token'] , true);

		if (isset($this->request->get['ipoid']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$purchase_order_info = $this->model_albertatest_purchase_order->getPurchaseOrder($this->request->get['ipoid']);
		...
		if (isset($this->request->post['vponumber'])) {
			...
			$temp_vponumber = $this->model_albertatest_purchase_order->getLastInsertedID();
		...
		$data['vendors'] = $this->model_albertatest_purchase_order->getVendors();
		$data['store'] = $this->model_albertatest_purchase_order->getStore();

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('albertatest/purchase_order_form', $data));
	}
	...
	public function get_vendor() {
		...
		if (isset($this->request->get['isupplierid'])) {
			$supplier = $this->model_albertatest_purchase_order->getVendor($this->request->get['isupplierid']);
	...
	public function get_search_items() {
		...
		if (!empty($this->request->get['term'])) {
			$items = $this->model_albertatest_purchase_order->getSearchItems($this->request->get['term']);
	...
	public function get_search_item() {
		...
		if (isset($this->request->get['iitemid']) && isset($this->request->get['ivendorid'])) {
			$item = $this->model_albertatest_purchase_order->getSearchItem($this->request->get['iitemid'],$this->request->get['ivendorid']);
			$json['item'] = $item;
		}
	...
	public function save_receive_item() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if(isset($this->request->post['items']) && count($this->request->post['items']) > 0){
				...
				$item_response = $this->model_albertatest_purchase_order->addSaveReceiveItem($this->request->post);
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			if(isset($this->request->post['vvendorid'])){
				...
				if(isset($this->request->files['import_invoice_file']) && $this->request->files['import_invoice_file']['name'] != ''){
                    ...
                    $vcategory = $this->model_albertatest_purchase_order->getCategories();
                    if(count($vcategory) > 0){
                    	$vcatcode = $vcategory[0]['vcategorycode'];
                    }

                    $vdepartment = $this->model_albertatest_purchase_order->getDepartments();
                    if(count($vdepartment) > 0){
                    	$vdepcode = $vdepartment[0]['vdepcode'];
                    }

                    $dtVend = $this->model_albertatest_purchase_order->getTax1();
                    if(count($dtVend) > 0){
                    	$vtax1 = "Y";
                    }

                    $DTVENDOR = $this->model_albertatest_purchase_order->getVendor($this->request->post['vvendorid']);
					...
					if ($handle1) {
					    ...
					    while (($strline = fgets($handle1)) !== false) {
					        ...
					        if ($FirstChar == "A"){
                            	...
                            	$dtCh = $this->model_albertatest_purchase_order->GetPurchaseOrderByInvoice($invoicenumber);
	                            ...
	                            $poid = $this->model_albertatest_purchase_order->insertPurchaseOrder($trnPurchaseorderdto);
                            }
                            ...
                            if ($FirstChar == "B"){
	                            ...
	                            $dtC = $this->model_albertatest_purchase_order->getItemByBarCode($vCode);
	                            ...
	                            if (count($dtC) == 0){
	                                ...
	                                if(isset($this->request->post['check_digit']) && $this->request->post['check_digit'] == 'Y'){
		                                ...
		                                $check_digit_barcode = $this->model_albertatest_purchase_order->calculate_upc_check_digit($vCode);

		                                if($check_digit_barcode != -1){

		                                	$old_vCode = $vCode.''.$check_digit_barcode;

		                                	$old_vCode = $this->model_albertatest_purchase_order->getItemByBarCode($old_vCode);

			                                if(count($old_vCode) > 0){
			                                	$old_vCode = $old_vCode['vbarcode'];
			                                	$dtC = $this->model_albertatest_purchase_order->getItemByBarCode($old_vCode);
			                                
				                                if(count($dtC) > 0){
				                                	$this->model_albertatest_purchase_order->updateBarcode($vCode,$old_vCode);
				                                }
			                                }else{
	                            ...
	                            if(count($dtC) > 0){
	                            	...
	                            	// }
	                            	$this->model_albertatest_purchase_order->updateItemStatus($dtC["iitemid"]);
	                            	//update item status

	                            	$iitemid = $dtC["iitemid"];
	                            	$dtI = $this->model_albertatest_purchase_order->getItemVendorByVendorItemCode($vvendoritemcode);
	                            	if(count($dtI) == 0){
	                            		...
	                            		$this->model_albertatest_purchase_order->insertItemVendor($mstItemVendorDto);
	                            	}
	                                ...
	                                $this->model_albertatest_purchase_order->InsertPurchaseDetail($trnPurchaseOrderDetaildto);
	                                ...
	                                $this->model_albertatest_purchase_order->createMissingitem($mst_missingitemDTO);
	                            	
	                            }
                            }
					    }

					    $this->model_albertatest_purchase_order->updatePurchaseOrderReturnTotal($nReturnTotal,$poid);
	...
	public function get_search_item_history() {
		...
		if (isset($this->request->get['search_item']) && $this->request->get['ivendorid']) {
			...
			$items = $this->model_albertatest_purchase_order->getSearchItemHistory($this->request->get['search_item'],$this->request->get['ivendorid'],$pre_items_id);
	...
	public function search_vendor_item_code() {
		...
		if (isset($this->request->get['search_item']) && $this->request->get['ivendorid']) {
			...
			$items = $this->model_albertatest_purchase_order->getSearchVendorItemCode($this->request->get['search_item'],$this->request->get['ivendorid'],$pre_items_id);
			...
			if(count($items_id) > 0){
				foreach ($items_id as $key => $item_id) {
					if(!in_array($item_id, $pre_items_id)){
						$json['items'][] = $this->model_albertatest_purchase_order->getSearchItem($item_id,$ivendorid);
					}
				}
			}
		}
	...
	public function get_item_history() {
		...
		if (isset($this->request->get['iitemid']) && isset($this->request->get['radio_search_by']) && isset($this->request->get['vitemcode'])) {
			$json = $this->model_albertatest_purchase_order->getSearchItemData($this->request->get['iitemid'],$this->request->get['radio_search_by'],$this->request->get['vitemcode'],null,null);
	...
	public function get_item_history_date() {
		...
		if (isset($this->request->get['iitemid']) && isset($this->request->get['start_date']) && isset($this->request->get['end_date']) && isset($this->request->get['vitemcode'])) {
			$json = $this->model_albertatest_purchase_order->getSearchItemData($this->request->get['iitemid'],null,$this->request->get['vitemcode'],$this->request->get['start_date'],$this->request->get['end_date']);
	...
	public function get_vendor_item() {
		...
		if (isset($this->request->get['isupplierid'])) {
			$json = $this->model_albertatest_purchase_order->getVendorItemData($this->request->get['isupplierid']);
		}
	...
	public function get_vendor_item_code_data() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if(count($items_id) > 0){
				foreach ($items_id as $key => $item_id) {
					if(!in_array($item_id, $pre_items_id)){
						$json['items'][] = $this->model_albertatest_purchase_order->getSearchItemVendorData($item_id['id'],$item_id['ivendorid']);
					}
				}
			}
		}
	...
	public function delete() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			$data = $this->model_albertatest_purchase_order->deletePurchaseOrder($temp_arr);
