<?php
class ControllerAlbertatestProfitLoss extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('albertatest/profit_loss');

		$this->document->setTitle($this->language->get('heading_title'));

    $this->load->model('administration/profit_loss');
		$this->load->model('albertatest/profit_loss');
  ...
  public function csv_export() {
    ...
    ini_set("memory_limit", "2G");
        ...
        $file_path = DIR_TEMPLATE."/albertatest/profit-loss-report.csv";

        $myfile = fopen( DIR_TEMPLATE."/albertatest/profit-loss-report.csv", "w");
  ...
  public function print_page() {
    ...
    $this->response->setOutput($this->load->view('albertatest/print_profit_loss_page', $data));
  }
  ...
  public function pdf_save_page() {
    ...
    $html = $this->load->view('albertatest/print_profit_loss_page', $data);
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
      ...
      if($this->request->post['report_by'] == 1){
        $reportsdata = $this->model_albertatest_profit_loss->getCategoriesReport($this->request->post);
        $data['drop_down_datas'] = $this->model_administration_profit_loss->getCategories();
      }elseif($this->request->post['report_by'] == 2){
        $reportsdata = $this->model_albertatest_profit_loss->getDepartmentsReport($this->request->post);
        $data['drop_down_datas'] = $this->model_administration_profit_loss->getDepartments();
      }else if($this->request->post['report_by'] == 3){
        $reportsdata = $this->model_albertatest_profit_loss->getItemGroupsReport($this->request->post);
      ...
      $this->session->data['data_grand_total_net_amt'] = $data['data_grand_total_net_amt'];
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('albertatest/profit_loss', 'token=' . $this->session->data['token'] . $url, true)
		);

    $data['report_ajax_data'] = $this->url->link('albertatest/profit_loss/report_ajax_data', 'token=' . $this->session->data['token'] . $url, true);
    $data['reportdata'] = $this->url->link('albertatest/profit_loss/reportdata', 'token=' . $this->session->data['token'] . $url, true);
    $data['print_page'] = $this->url->link('albertatest/profit_loss/print_page', 'token=' . $this->session->data['token'] . $url, true);
    $data['pdf_save_page'] = $this->url->link('albertatest/profit_loss/pdf_save_page', 'token=' . $this->session->data['token'] . $url, true);
    $data['csv_export'] = $this->url->link('albertatest/profit_loss/csv_export', 'token=' . $this->session->data['token'] . $url, true);
    ...
    $data['store_name'] = $this->session->data['storename'];
		...
		$this->response->setOutput($this->load->view('albertatest/profit_loss_list', $data));
	}
  ...
  public function report_ajax_data() {
    ...
    if ($this->request->server['REQUEST_METHOD'] == 'POST') {
      ...
      if($temp_arr['report_by'] == 1){
        $data = $this->model_albertatest_profit_loss->ajaxDataReportCategory($temp_arr);
      }else if($temp_arr['report_by'] == 2){
        $data = $this->model_albertatest_profit_loss->ajaxDataReportDepartment($temp_arr);
      }else if($temp_arr['report_by'] == 3){
        $data = $this->model_albertatest_profit_loss->ajaxDataReportItemGroup($temp_arr);
      }else{
        $data = $this->model_albertatest_profit_loss->ajaxDataReportDepartment($temp_arr);
      }
