...
class ModelApiItemMovementReport extends Model {
	...
	public function getItemMovementReport($data) {
		...
		$query_item = $this->db2->query("SELECT a.iitemid, a.vitemtype, a.vbarcode, a.vitemname, a.npack, CASE WHEN a.NPACK = 1 or (a.npack is null)   then a.IQTYONHAND else (Concat(cast(((a.IQTYONHAND div a.NPACK )) as signed), '  (', Mod(a.IQTYONHAND,a.NPACK) ,')') ) end as IQTYONHAND FROM mst_item as a WHERE a.vbarcode='". $this->db->escape($data['search_vbarcode']) ."'")->row;

		$sql = "SELECT ifnull(SUM(trn_sd.ndebitqty),0) as items_sold, date_format(trn_s.dtrandate,'%m-%Y') as dtrandate, trn_sd.npack FROM trn_salesdetail trn_sd , trn_sales trn_s WHERE trn_s.vtrntype='Transaction' AND  trn_sd.ndebitqty > 0 AND trn_s.isalesid=trn_sd.isalesid AND trn_sd.vitemcode='". $this->db->escape($data['search_vbarcode']) ."' AND (date_format(trn_s.dtrandate,'%Y') BETWEEN '".$start_date."' AND '".$end_date."') GROUP BY date_format(trn_s.dtrandate,'%m-%d-%Y')";

		$query_data = $this->db2->query($sql)->rows;

		$sql1 = "SELECT ifnull(SUM(tpd.itotalunit),0) as items_receive, DATE_FORMAT(tp.dcreatedate,'%m-%Y') as dcreatedate, tpd.npackqty as npack FROM trn_purchaseorderdetail tpd LEFT JOIN trn_purchaseorder tp ON (tpd.ipoid = tp.ipoid) WHERE tpd.vitemid='". $this->db->escape($data['search_iitemid']) ."' AND (date_format(tp.dcreatedate,'%Y') BETWEEN '".$start_date."' AND '".$end_date."') GROUP BY date_format(tp.dcreatedate,'%m-%d-%Y')";
