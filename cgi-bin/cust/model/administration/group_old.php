...
class ModelAdministrationGroup extends Model {
	...
	public function getLeftItems($data = array()) {
		...
		if(count($data) > 0){
			...
			$query = $this->db2->query("SELECT iitemid,vbarcode,vitemname,nsaleprice,dunitprice FROM mst_item WHERE estatus='Active'");
			$return = $query->rows;
		}
		return $return;
	}
    ...
    public function getSearchItems($data = array()) {
    	...
    	if(isset($data['search_by']) && $data['search_by'] == 'vbarcode'){
    		if($item_ids != ''){
    			$query = $this->db2->query("SELECT iitemid,vbarcode,vitemname,nsaleprice,dunitprice FROM mst_item WHERE vbarcode LIKE  '%" .$this->db->escape($data['search_val']). "%' AND estatus='Active' AND iitemid NOT IN($item_ids)");
    		}else{
    			$query = $this->db2->query("SELECT iitemid,vbarcode,vitemname,nsaleprice,dunitprice FROM mst_item WHERE vbarcode LIKE  '%" .$this->db->escape($data['search_val']). "%' AND estatus='Active'");
    		}
    		
    	}else{
    		if($item_ids != ''){
    			$query = $this->db2->query("SELECT iitemid,vbarcode,vitemname,nsaleprice,dunitprice FROM mst_item WHERE vitemname LIKE  '%" .$this->db->escape($data['search_val']). "%' AND estatus='Active' AND iitemid NOT IN($item_ids)");
    		}else{
    			$query = $this->db2->query("SELECT iitemid,vbarcode,vitemname,nsaleprice,dunitprice FROM mst_item WHERE vitemname LIKE  '%" .$this->db->escape($data['search_val']). "%' AND estatus='Active'");
    		}
    	}
