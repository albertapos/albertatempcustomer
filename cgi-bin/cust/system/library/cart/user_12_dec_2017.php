...
//$store = $this->db->query("SELECT id,name,db_name,db_username,db_password,db_hostname FROM stores WHERE id='".$this->db->escape($SID). "'");	
	...
	public function login($username, $password) {
		...
		if ($user_query->num_rows) {
			...
			if (password_verify($this->db->escape($password), $hash)) {
				...
				$store_count = $this->changeStore($this->session->data['user_id'],$this->session->data['role_id']);

				if(count($store_count) > 0){
                    return true;
                }else{
                    return false;
                }
	...
	public function changeStore($userId,$roleId,$storeId = null) {
        ...
        if($roleId == 1){
            ...
            if(!empty($storeId)){
                $store = $this->db->query("SELECT id,name,db_name,db_username,db_password,db_hostname,webadmin FROM stores WHERE id='".$storeId."' AND webadmin=1");
            }else{
                $store = $this->db->query("SELECT s.id,s.name,s.db_name,s.db_username,s.db_password,s.db_hostname,s.webadmin FROM stores as s,user_stores as us WHERE s.id=us.store_id AND us.user_id='". (int)$userId ."' AND s.webadmin=1 ORDER BY s.id");
            }

            $store_arr = $this->db->query("SELECT s.id,s.name,s.db_name,s.db_username,s.db_password,s.db_hostname,s.webadmin FROM stores as s,user_stores as us WHERE s.id=us.store_id AND us.user_id='". (int)$userId ."' AND s.webadmin=1 ORDER BY s.id")->rows;
            ...
            if(!empty($storeId)){
                ...
                $store = $this->db->query("SELECT s.id,s.name,s.db_name,s.db_username,s.db_password,s.db_hostname,s.webadmin FROM stores as s,sales_stores as ss, users as u WHERE u.id=ss.user_id AND ss.user_id='". (int)$userId ."' AND s.webadmin=1 ORDER BY s.id");
            }

            $store_arr = $this->db->query("SELECT s.id,s.name,s.db_name,s.db_username,s.db_password,s.db_hostname,s.webadmin FROM stores as s,sales_stores as ss, users as u WHERE u.id=ss.user_id AND ss.user_id='". (int)$userId ."' AND s.webadmin=1 ORDER BY s.id")->rows;
            ...
            if(!empty($storeId)){
                ...
                $store = $this->db->query("SELECT * FROM stores WHERE id in($new_s_ids_string) AND webadmin=1");
            }

            $store_arr = $this->db->query("SELECT id,name FROM stores WHERE id in($new_s_ids_string) AND webadmin=1")->rows;
            ...
            if(!empty($storeId)){
                ...
                $store = $this->db->query("SELECT DISTINCT s.id,s.name,s.db_name,s.db_username,s.db_password,s.db_hostname,s.webadmin FROM stores as s,sales_stores as ss, users as u WHERE u.id=ss.user_id AND ss.user_id='". (int)$userId ."' AND s.webadmin=1 ORDER BY s.id");
            }

            $store_arr = $this->db->query("SELECT DISTINCT s.id,s.name,s.db_name,s.db_username,s.db_password,s.db_hostname,s.webadmin FROM stores as s,sales_stores as ss, users as u WHERE u.id=ss.user_id AND ss.user_id='". (int)$userId ."' AND s.webadmin=1 ORDER BY s.id")->rows;
