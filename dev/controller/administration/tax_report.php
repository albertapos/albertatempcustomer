<?php
class ControllerAdministrationTaxReport extends Controller {
	private $error = array();

	public function index() {
    $this->load->language('administration/tax_report');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('api/tax_report');
  ...
  public function csv_export() {
    ...
    ini_set("memory_limit", "2G");
      ...
      $data_row .= "Store Phone:".$this->session->data['storephone'].PHP_EOL.PHP_EOL;
      ...
      $file_path = DIR_TEMPLATE."/administration/tax-report.csv";

      $myfile = fopen( DIR_TEMPLATE."/administration/tax-report.csv", "w");
  ...
  public function print_page() {
    ...
    $this->response->setOutput($this->load->view('administration/print_tax_report_page', $data));
  }
  ...
  public function pdf_save_page() {
    ...
    $html = $this->load->view('administration/print_tax_report_page', $data);
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
      ...
      $data['reports'] = $this->model_api_tax_report->getTaxReport($this->request->post);
      ...
      $this->session->data['p_end_date'] = $data['p_end_date'];
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('administration/tax_report', 'token=' . $this->session->data['token'] . $url, true)
		);

    $data['reportdata'] = $this->url->link('administration/tax_report/reportdata', 'token=' . $this->session->data['token'] . $url, true);
    $data['print_page'] = $this->url->link('administration/tax_report/print_page', 'token=' . $this->session->data['token'] . $url, true);
    $data['pdf_save_page'] = $this->url->link('administration/tax_report/pdf_save_page', 'token=' . $this->session->data['token'] . $url, true);
    $data['csv_export'] = $this->url->link('administration/tax_report/csv_export', 'token=' . $this->session->data['token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('administration/tax_report_list', $data));
	}
