...
class ControllerApiSalesReport extends Controller {
	...
	public function index() {
		...
		if (($this->session->data['token'] == $this->request->get['token']) && ($this->session->data['sid'] == $this->request->get['sid'])) {
			...
			$data = $this->model_api_sales_report->getSalesReport($temp_arr);
	...
	public function print_data() {
		...
		if (($this->session->data['token'] == $this->request->get['token']) && ($this->session->data['sid'] == $this->request->get['sid']) && ($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			$data['sales_header'] = $this->model_api_sales_report->getSalesById($salesid);

			$data['sales_detail'] = $this->model_api_sales_report->getSalesPerview($salesid);
			
			$data['sales_tender'] = $this->model_api_sales_report->getSalesByTender($salesid);
