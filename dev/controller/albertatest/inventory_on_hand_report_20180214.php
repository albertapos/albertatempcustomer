...
class ControllerAdministrationInventoryOnHandReport extends Controller {
  ...
  public function csv_export() {
        ...
        if(count($data['reports']) > 0){
            if($data['selected_report'] == 1){
              $data_row .= 'Supplier,Category,Item,QOH,Cost Value,Total Cost Value,Retail Value,Total Retail Value'.PHP_EOL;
            }else{
              $data_row .= 'Supplier,Department,Item,QOH,Cost Value,Total Cost Value,Retail Value,Total Retail Value'.PHP_EOL;
            }
            ...
            foreach ($data['reports'] as $key => $value) {
              ...
              foreach ($value as $k => $v){
                ...
                $data_row .= str_replace(',',' ',$v['suppliername']).','.str_replace(',',' ',$v['vname']).','.str_replace(',',' ',$v['itemname']).','.$v['iqtyonhand'].','.number_format((float)$v['cost'], 2, '.', '').','. number_format((float)$tot_value, 2, '.', '').','.number_format((float)$v['price'], 2, '.', '').','.number_format((float)$tot_ret_value, 2, '.', '') .PHP_EOL;
              }
  ...
  public function pdf_save_page() {
    ...
    $data['get_json_data'] = $this->url->link('administration/inventory_on_hand_report/get_json_data', 'token=' . $this->session->data['token'] . $url, true);
  ...
  public function get_json_data(){
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
      $temp['report_data'] = array("ALL");
      
      $reportsdata = $this->model_api_inventory_on_hand_report->getCategoriesReport($temp);
