...
class ControllerAdministrationUsers extends Controller {
	...
	protected function getList() {
		...
		$data['current_url'] = $this->url->link('administration/users', 'token=' . $this->session->data['token'], true);

		$data['searchuser'] = $this->url->link('administration/users/search_user', 'token=' . $this->session->data['token'], true);
		...
		$users_total = $this->model_administration_users->getTotalUsers($filter_data);
  	...
  	protected function getForm() {
		...
		$data['status_arr']['Active'] = 'Active';
		$data['status_arr']['Inactive'] = 'Inactive';
		...
		if (isset($this->error['estatus'])) {
			$data['error_estatus'] = $this->error['estatus'];
		} else {
			$data['error_estatus'] = '';
		}
		...
		if (isset($this->request->post['vpassword'])) {
			...
			$data['vpassword'] = $this->model_administration_users->decodePassword($users_info['vpassword']);
		} else {
			$data['vpassword'] = '';
		}
	...
	protected function validateForm() {
		...
		if ($this->request->post['estatus'] != '') {
			if ($this->request->post['estatus'] == 'Inactive' && $this->request->post['vusertype'] == 'Admin') {
				$this->error['estatus']= 'You Can not Inactive Admin User';
			}
		}
	...
	public function search_user(){
		...
		if(isset($this->request->get['term']) && !empty($this->request->get['term'])){
			...
			$datas = $this->model_api_users->getUsersSearch($this->request->get['term']);

			foreach ($datas as $key => $value) {
				$temp = array();
				$temp['iuserid'] = $value['iuserid'];
				$temp['vusername'] = $value['vfname'].' '.$value['vlname'];
				$return[] = $temp;
			}
		}
