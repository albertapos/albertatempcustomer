                  ...
                  <tr >
                    ...
                    <td colspan="8" style="padding:0px">
                      <table class="table" style="margin-top: 3px;margin-bottom: 3px;">
                        <thead>
                          <th colspan="8" class="text-uppercase header" data-vsuppliercode="<?php echo $value[0]['vsuppliercode']; ?>" style="cursor: pointer;"><?php echo $key; ?> <span class="sign"></span></th>
                        </thead>
...
$(document).on('click', '.header', function(event) {
  ...
  var report_vsuppliercode = $(this).attr('data-vsuppliercode');
  
  var data_post = {report_by:report_by,report_data:report_data,report_vsuppliercode:report_vsuppliercode};
  ...
  if($(this).hasClass('expand')){
    $.ajax({
      ...
      dataType: 'json',
        ...
        current_header.parent().parent().parent().find('tbody').empty();
        var html = '';
        if(response){
          var total_qty = 0;
          ...
          $.each(response,function(i,v){
            ...
            var tot_ret_value = parseInt(v.iqtyonhand) * parseFloat(v.price).toFixed(2);

            total_total_retail_value = total_total_retail_value + tot_ret_value;

            total_qty = total_qty + parseInt(v.iqtyonhand);
            total_total_cost = total_total_cost + parseFloat(v.cost).toFixed(2);
            ...
            html += '</td>';
            html += '<td style="width: 10%;" class="text-right">';
            html += parseFloat(v.price).toFixed(2);
            html += '</td>';
            html += '<td style="width: 10%;" class="text-right">';
          ...
          current_header.parent().parent().parent().find('tbody').append(html);
        }
    });
  }
  
  if(!$(this).hasClass('expand')){
    $(this).parent().parent().parent().find('tbody').empty();
  }
