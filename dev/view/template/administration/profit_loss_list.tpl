            ...
            <table class="table table-bordered table-striped table-hover" style="border:none;">
              ...
              <tbody>
                ...
                ?>
                      ...
                      <td colspan="9" style="padding:0px">
                        ...
                        <thead>
                          <tr class="search_header" style="cursor: pointer;" data-id="<?php echo $key; ?>" data-TotalQty="<?php echo $value['TotalQty'];?>" data-NetAmt="<?php echo $value['NetAmt'];?>">
                            ...
                            <th style="width: 10%;" class="text-right"><?php echo number_format((float)$value['Amount'], 2, '.', '') ; ?></th>
                            <?php

                            if($value['TotCostPrice'] != '0.00'){
                              $markup = (($value['Amount']/$value['TotCostPrice']) * 100);
                            }else{
                              $markup = 100.00;
                            }

                            ?>
                            <th style="width: 10%;" class="text-right"><?php echo number_format((float)$markup, 2, '.', '') ; ?>%</th>
                  ...
                  <tr>
                    <?php
                      if($data_grand_total_total_cost != '0.00'){
                        $tot_mark_up = ($data_grand_total_total_amount / $data_grand_total_total_cost) * 100;
                      }else{
                        $tot_mark_up = 100;
                      }
                    ?>
      ...
      dataType: 'json',
        ...
        if(response){
          ...
          $.each(response,function(i,v){
            ...
            var NetAmt = (parseFloat(v.TOTUNITPRICE));
            ...
            html += '</td>';
            html += '<td style="width: 10%;" class="text-right">';
            html += parseFloat(v.Amount).toFixed(2);
            html += '</td>';
            html += '<td style="width: 10%;" class="text-right">';
          ...
          html += '<tr>';
            ...
            html += '<td style="width: 10%;" class="text-right">'+ total_gross_profit.toFixed(2) +'</td>';
            
            if(total_total_cost != '0.00'){
              var markup = ((total_gross_profit / total_total_cost) * 100).toFixed(2);
            }else{
              var markup = (100).toFixed(2);
            }
            html += '<td style="width: 10%;" class="text-right">'+ markup +'%</td>';
