<?php
class ModelAlbertatestZeroMovementReport extends Model {
	...
	public function getGroupsReport($data) {
		...
		if(in_array('ALL', $data['report_data'])){
			...
			$sql = "SELECT c.vcompanyname as suppliername,a.vitemname as itemname, CASE WHEN npack = 1 or (npack is null)   then iqtyonhand else (Concat(cast((a.iqtyonhand/a.npack ) as signed), '  (' , Mod(a.iqtyonhand,a.npack) , ')') )  end as vqty, n.vitemgroupname as vname,Case When (nUnitCost) is null then 0 else  nUnitCost end as cost, a.dUnitPrice as price,a.iqtyonhand,(select shelfname from mst_shelf d where a.shelfid=d.Id) as shelf, (select shelvingname from mst_shelving e where a.shelvingid=e.id) as shelving, (select aislename from mst_aisle f where a.aisleid=f.Id) as aisle FROM mst_item a,itemgroupdetail m,itemgroup n,mst_supplier c  where a.vbarcode=m.vsku and m.iitemgroupid=n.iitemgroupid and a.vsuppliercode =c.vsuppliercode and a.vitemtype != 'Kiosk' and (a.nUnitCost*npack) > a.dUnitPrice and vitemname is not null and a.vbarcode not in (select a.vitemcode from trn_salesdetail a,trn_sales b where a.isalesid=b.isalesid and vtrntype='Transaction' and date_format(b.dtrandate,'%Y-%m-%d') between str_to_date('". $data['start_date'] ."','%m-%d-%Y') and str_to_date('". $data['end_date'] ."','%m-%d-%Y'))";

		}else{
			$vgroups = implode(',', $data['report_data']);

			$sql = "SELECT c.vcompanyname as suppliername,a.vitemname as itemname, CASE WHEN npack = 1 or (npack is null)   then iqtyonhand else (Concat(cast((a.iqtyonhand/a.npack ) as signed), '  (' , Mod(a.iqtyonhand,a.npack) , ')') )  end as vqty, n.vitemgroupname as vname,Case When (nUnitCost) is null then 0 else  nUnitCost end as cost, a.dUnitPrice as price,a.iqtyonhand,(select shelfname from mst_shelf d where a.shelfid=d.Id) as shelf, (select shelvingname from mst_shelving e where a.shelvingid=e.id) as shelving, (select aislename from mst_aisle f where a.aisleid=f.Id) as aisle FROM mst_item a,itemgroupdetail m,itemgroup n,mst_supplier c  where a.vbarcode=m.vsku and m.iitemgroupid=n.iitemgroupid and a.vsuppliercode =c.vsuppliercode and m.iitemgroupid in ($vgroups) and a.vitemtype != 'Kiosk' and (a.nUnitCost*npack) > a.dUnitPrice and vitemname is not null and a.vbarcode not in (select a.vitemcode from trn_salesdetail a,trn_sales b where a.isalesid=b.isalesid and vtrntype='Transaction' and date_format(b.dtrandate,'%Y-%m-%d') between str_to_date('". $data['start_date'] ."','%m-%d-%Y') and str_to_date('". $data['end_date'] ."','%m-%d-%Y'))";
		}
