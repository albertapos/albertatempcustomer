<?php
class ModelAlbertatestCreditCardReport extends Model {
	...
	public function getCreditCardReport($data) {
		...
		if (!empty($data['credit_card_number'])) {
            $sql .= " AND trn_mps.vcardusage='". $data['credit_card_number'] ."'";
        }

        if (!empty($data['credit_card_amount'])) {
            $sql .= " AND trn_mps.nauthamount='". $data['credit_card_amount'] ."'";
        }
