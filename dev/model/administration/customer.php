...
class ModelAdministrationCustomer extends Model {
	...
	public function getCustomers($data = array()) {
		...
		if (!empty($data['searchbox'])) {
			$sql .= " Where vfname LIKE  '%" .$this->db->escape($data['searchbox']). "%' OR vlname LIKE  '%" .$this->db->escape($data['searchbox']). "%' OR vaccountnumber LIKE  '%" .$this->db->escape($data['searchbox']). "%' OR vcustomername LIKE  '%" .$this->db->escape($data['searchbox']). "%' OR estatus LIKE  '%" .$this->db->escape($data['searchbox']). "%'";
		}
	...
	public function getTotalCustomers($data) {
		...
		if (!empty($data['searchbox'])) {
			$sql .= " where (vfname LIKE '" . $this->db->escape($data['searchbox']) . "%' OR vlname LIKE '" . $this->db->escape($data['searchbox']) . "%' OR vcustomername LIKE '" . $this->db->escape($data['searchbox']) . "%' OR vaccountnumber LIKE '" . $this->db->escape($data['searchbox']) . "%')";
		}
