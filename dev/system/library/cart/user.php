...
//$store = $this->db->query("SELECT id,name,db_name,db_username,db_password,db_hostname FROM stores WHERE id='".$this->db->escape($SID). "'");	
	...
	public function changeStore($userId,$roleId,$storeId = null) {
		...
		if($roleId == 1){
			if(!empty($storeId)){
				$store = $this->db->query("SELECT id,name,address,phone,db_name,db_username,db_password,db_hostname,webadmin FROM stores WHERE id='".$storeId."'");
			}else{
				$store = $this->db->query("SELECT id,name,address,phone,db_name,db_username,db_password,db_hostname,webadmin FROM stores ORDER BY id");
			}

			$store_arr = $this->db->query("SELECT id,name,address,phone,db_name,db_username,db_password,db_hostname,webadmin FROM stores ORDER BY id")->rows;
			...
			if(isset($store)){
				...
				$this->session->data['db_database2'] = $store->row['db_name'];
				$this->session->data['storename'] = $store->row['name'];
				$this->session->data['storeaddress'] = $store->row['address'];
				$this->session->data['storephone'] = $store->row['phone'];
			...
			if(!empty($storeId)){
				...
				$store = $this->db->query("SELECT s.id,s.name,s.address,s.phone,s.db_name,s.db_username,s.db_password,s.db_hostname,s.webadmin FROM stores as s,user_stores as us WHERE s.id=us.store_id AND us.user_id='". (int)$userId ."' ORDER BY s.id");
			}
            //die("SELECT s.id,s.name,s.address,s.phone1,s.db_name,s.db_username,s.db_password,s.db_hostname,s.webadmin FROM stores as s,user_stores as us WHERE s.id=us.store_id AND us.user_id='". (int)$userId ."' ORDER BY s.id");
			$store_arr = $this->db->query("SELECT s.id,s.name,s.address,s.phone,s.db_name,s.db_username,s.db_password,s.db_hostname,s.webadmin FROM stores as s,user_stores as us WHERE s.id=us.store_id AND us.user_id='". (int)$userId ."' ORDER BY s.id")->rows;
			...
			if(!empty($storeId)){
				$store = $this->db->query("SELECT id,name,s.address,s.phone,db_name,db_username,db_password,db_hostname,webadmin FROM stores WHERE id='".$storeId."'");
			}else{
				$store = $this->db->query("SELECT s.id,s.name,s.address,s.phone,s.db_name,s.db_username,s.db_password,s.db_hostname,s.webadmin FROM stores as s,sales_stores as ss, users as u WHERE u.id=ss.user_id AND ss.user_id='". (int)$userId ."' ORDER BY s.id");
			}

			$store_arr = $this->db->query("SELECT s.id,s.name,s.address,s.phone,s.db_name,s.db_username,s.db_password,s.db_hostname,s.webadmin FROM stores as s,sales_stores as ss, users as u WHERE u.id=ss.user_id AND ss.user_id='". (int)$userId ."' ORDER BY s.id")->rows;
			...
			$store_arr = $this->db->query("SELECT id,name,address,phone FROM stores WHERE id in($new_s_ids_string)")->rows;
			...
			if(!empty($storeId)){
				...
				$store = $this->db->query("SELECT DISTINCT s.id,s.name,s.address,s.phone,s.db_name,s.db_username,s.db_password,s.db_hostname,s.webadmin FROM stores as s,sales_stores as ss, users as u WHERE u.id=ss.user_id AND ss.user_id='". (int)$userId ."' ORDER BY s.id");
			}

			$store_arr = $this->db->query("SELECT DISTINCT s.id,s.name,s.address,s.phone,s.db_name,s.db_username,s.db_password,s.db_hostname,s.webadmin FROM stores as s,sales_stores as ss, users as u WHERE u.id=ss.user_id AND ss.user_id='". (int)$userId ."' ORDER BY s.id")->rows;
