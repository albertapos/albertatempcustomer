        ...
        <br>
          ...
          <div class="table-responsive">
            ...
            <?php if ($items) { ?>
              <thead>
                <tr>
                  <th style="width: 1px;" class="text-center"><!-- <input type="checkbox" onclick="$('input[name*=\'selected_items_id\']').prop('checked', this.checked);" /> --></th>
...
$(document).on('change', 'input:radio[name="search_radio"]', function(event) {
 ...
 $('[data-target="#myModal"]').click(function(event) {
    ...
    checked_items_val = $.parseJSON($.cookie("items_total_ids"));
  ...
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      <form action="<?php echo $edit_list;?>" method="post" id="form_item_update">
        ...
        <input type="hidden" name="search_find_btn" value="<?php echo isset($search_radio)? $search_find : '' ;?>">
              ...
              <div class="col-md-5 span_field" style="padding-left:0px;padding-right:0px;">
                ...
                <p>
                  <span style="width:35%;">
                    ...
                    <input type="checkbox" value="Y" name="update_dunitprice_increment">&nbsp;&nbsp;increment price
                  </span>&nbsp;&nbsp;
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    if(!$.cookie("items_total_ids")){
      var total_ids = [];
      $.cookie("items_total_ids", JSON.stringify([]));

      $('input[name="selected_items_id[]"]').prop( 'checked', true );

      <?php if(isset($items_total_ids) && count($items_total_ids) > 0){?>
        <?php foreach($items_total_ids as $key => $items_total_id){?>

          if($.inArray(<?php echo $items_total_id;?>,total_ids) == -1){
              total_ids.push(<?php echo $items_total_id;?>);
          };
      ...
      console.log(1);
      $.cookie("items_total_ids", JSON.stringify(total_ids));
      console.log($.parseJSON($.cookie("items_total_ids")));

    }else{
      console.log($.parseJSON($.cookie("items_total_ids")));
      var total_ids = $.parseJSON($.cookie("items_total_ids"));
      console.log(total_ids);
      console.log(2);

      if(total_ids.length > 0){
        $.each(total_ids, function(i, val){
          $("#item input[value='" + val + "']").prop('checked', true);
        });
      }else{
        $('input[name="selected_items_id[]"]').prop( 'checked', true );
      }
  ...
  $(document).on('change', 'input[name="selected_items_id[]"]', function(event) {
    ...
    if($(this).is(':checked')){
      var total_ids = $.parseJSON($.cookie("items_total_ids"));
      if($.inArray(parseInt($(this).val()),total_ids) == -1){
        total_ids.push(parseInt($(this).val()));
        $.cookie("items_total_ids", JSON.stringify(total_ids));
      }
    }else{
     var total_ids = $.parseJSON($.cookie("items_total_ids"));
     total_ids.splice($.inArray(parseInt($(this).val()), total_ids), 1);
     $.cookie("items_total_ids", JSON.stringify(total_ids));
    }
    console.log(total_ids);
  ...
  $(document).on('click', '#cancel_button, #menu li a, .breadcrumb li a, input[name="search_filter"], #header > li', function() {
    // $.cookie("items_total_ids", JSON.stringify([])); //set cookie tab
    $.removeCookie('items_total_ids');
  });
</script>