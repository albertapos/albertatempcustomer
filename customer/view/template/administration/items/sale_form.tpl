        ...
        <?php } ?>
          ...
          <div class="row">
            <div class="col-md-6" style="padding-top:10px;" id="div_date">
              <div class="form-group">
                <?php
                  if(isset($dstartdatetime) && !empty($dstartdatetime) && $dstartdatetime != "0000-00-00 00:00:00"){
                  ...
                  if(isset($denddatetime) && !empty($denddatetime) && $denddatetime != "0000-00-00 00:00:00"){
                ...
                <table class="table table-bordered table-hover" style="padding:0px; margin:0px;" >
                  <thead>
                    <tr>
                      ...
                      <td class="text-right" id="td_discount" style="width:5px">Discd. Price</td>
    ...
    $.ajax({
      ...
      if(response.right_items_arr){
        ...
        $.each(response.right_items_arr, function(i, v) {
          ...
          right_items_html += '<td class="text-right"><div class="unitPrice">'+v.dunitprice+'</div><input type="hidden" name="items['+window.index_item+'][dunitprice]" class="i_dunitprice" value="'+v.dunitprice+'"></td>';
          right_items_html += '<td class="text-right"><span class="s_nsaleprice" contenteditable="true">'+display_nsaleprice+'</span><input type="hidden" name="items['+window.index_item+'][nsaleprice]" class="i_nsaleprice" value="'+ display_nsaleprice +'"></td>';
          right_items_html += '</tr>';
          window.index_item++;
        });
...
/*$('div.unitPrice').change(function(){
    console.log("Text changed");
});*/

$(document).on('DOMSubtreeModified', ".unitPrice", function() {
    change = $(this).text();
...
$(document).on('DOMSubtreeModified', ".s_nsaleprice", function() {
    change = $(this).text();
    $(this).next().val(change);
    console.log(change);
});
...
/*$(document).on('change', 'div.unitPrice', function(event) {
  event.preventDefault();
  console.log("Text changed");
});*/
    ...
    $.getJSON(display_items_url, function(result){
      ...
      if(result.edit_right_items){
        ...
        $.each(result.edit_right_items, function(i, v) {
          ...
          right_items_html += '<td>'+v.vitemname+'<input type="hidden" name="items['+window.index_item+'][vitemname]" value="'+v.vitemname+'"></td>';
          right_items_html += '<td class="text-right"><div class="unitPrice">'+v.dunitprice+'</div><input type="hidden" name="items['+window.index_item+'][dunitprice]" class="i_dunitprice" value="'+v.dunitprice+'"></td>';
          right_items_html += '<td class="text-right"><span class="s_nsaleprice" contenteditable="true">'+v.nsaleprice+'</span><input type="hidden" name="items['+window.index_item+'][nsaleprice]" class="i_nsaleprice" value="'+v.nsaleprice+'"></td>';
          right_items_html += '</tr>';
          window.index_item++;
        });
