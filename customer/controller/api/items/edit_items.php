<?php
class ControllerApiItemsEditItems extends Controller {
	private $error = array();
	...
	public function index() {
		...
		if (($this->session->data['token'] == $this->request->get['token']) && ($this->session->data['sid'] == $this->request->get['sid'])) {
			...
			$data = $this->model_api_items_edit_items->getItems();
	...
	public function edit_list() {
		...
		if (($this->session->data['token'] == $this->request->get['token']) && ($this->session->data['sid'] == $this->request->get['sid']) && ($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			$data = $this->model_api_items_edit_items->editlistItems($temp_arr);
	...
	public function search() {
		...
		if (($this->session->data['token'] == $this->request->get['token']) && ($this->session->data['sid'] == $this->request->get['sid']) && ($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			$data = $this->model_api_items_edit_items->getItems($temp_arr);
