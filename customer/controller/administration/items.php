	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$temp_arr = array(
						        ...
						        "vitemname" => htmlspecialchars_decode($this->request->post['vitemname']),
						        ...
						        "vdescription" => htmlspecialchars_decode($this->request->post['vdescription']),
						        "dlastsold" => null,
						...
						if(count($values) != 20){
							...
							if(strlen($itemcode) > 0 && strlen($itemname)){
								...
								if(count($checkItemCode) == 0){
	                                ...
	                                $unitcount = $this->model_api_units->getUnitByName($unit);
	                                ...
	                                //Supplier Info
	                                $suppliercount = $this->model_api_vendor->getVendorByName($supplier);
	                                ...
	                                //Group Info
	                                $groupcount = $this->model_api_group->getGroupByName($groupname);
                                

	                                if(count($groupcount) == 0){
	                                	$group = $this->model_api_group->addGroupByName($groupname,$itemcode);
	                                } else {
	                                    $group = $groupcount['vsuppliercode'];
	                                }
	                                ...
	                                $ageverificationcount = $this->model_api_age_verification->getAgeVerificationByValue($ageverification);
	                                ...
	                                $data['itemimage'] = $picture;
	                                
	                                
//$file_path1 = DIR_TEMPLATE."/administration/error_log_import_item.txt";

//$myfile1 = fopen( DIR_TEMPLATE."/administration/error_log_import_item.txt", "a");
