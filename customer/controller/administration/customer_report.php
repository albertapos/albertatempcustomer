...
class ControllerAdministrationCustomerReport extends Controller {
  ...
  public function customer_view($id=0) {
	...
	$cus_data = $this->model_administration_customer->getCustomer($icustomerid);

	$html='';
	$html .= '<div class="row">';
	$html .= '<div class="col-md-12 text-center">';
	$html .= '<h3>CUSTOMER STATEMENT</h3>';
	$html .= '</div>';
	$html .= '<div class="col-md-12">';
	$html .= '<p style="margin-bottom:2px;"><strong>Store Name: </strong>'.$store_info['vstorename'].'</p>';
	$html .= '<p style="margin-bottom:2px;"><strong>Store Address: </strong>'.$store_info['vaddress1'].'</p>';
	$html .= '<p style="margin-bottom:2px;"><strong></strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$store_info['vcity'].', '.$store_info['vstate'].' '.$store_info['vzip'].' </p>';
	$html .= '<p style="margin-bottom:2px;"><strong>Store Phone: </strong>'.$this->session->data['storephone'].'</p>';
	$html .= '</div>';
	$html .= '</div>';
	$html .= '<br>';
	$html .= '<div class="row" style="width:100%;">';
	$html .= '<div class="col-md-8 text-left" style="width:66%;display:inline-block;">';
	$html .= '<p style="margin-bottom:2px;"><strong>'.$cus_data['vfname'].' '.$cus_data['vlname'].'</strong></p>';
	$html .= '<p style="margin-bottom:2px;"><strong>'.$cus_data['vaddress1'].'</strong></p>';
	$html .= '<p style="margin-bottom:2px;"><strong>'.$cus_data['vcity'].', '.$cus_data['vstate'].' '.$cus_data['vzip'].' </strong></p>';
	$html .= '</div>';
	$html .= '<div class="col-md-4" style="width:33%;display:inline-block;">';
	$html .= '<p style="margin-bottom:2px;"><strong>Date:</strong> '.date('F d, Y').'</p>';
	$html .= '<p style="margin-bottom:2px;"><strong>Account:</strong> '.$cus_data['vaccountnumber'].'</p>';
	$html .= '<p style="margin-bottom:2px;"><strong>Phone:</strong> '.$cus_data['vphone'].'</p>';
	$html .= '</div>';
	$html .= '</div>';
	$html .= '<hr style="border-top:1px solid #999;">';
	$html.='<table class="table table-bordered table-striped table-hover" style="border:none;">';
	$html.='<thead><tr>';	  
	$html.='<th class="text-left">Tran. Date</th>';
	//$html.='<th>Check Number</th>';
	//$html.='<th>Bank Name</th>';
	$html.='<th>Tran. Type</th>';
	$html.='<th class="text-right">Credit Amt.</th>';
	$html.='<th class="text-right">Debit Amt.</th>';
	$html.='<th class="text-right">Balance Amt.</th>';
	$html.='</tr></thead>';
	$html.='<tbody>';	
	foreach($sales_tender as $value){
		$html.='<tr>';
		$html.='<td class="text-left">'.$value['dtrandate'].'</td>';
		//$html.='<td>'.$value['vchecknumber'].'</td>';
		//$html.='<td>'.$value['vbankname'].'</td>';
		$html.='<td>'.$value['vtrantype'].'</td>';
		$html.='<td class="text-right">'.$value['ncreditamt'].'</td>';
		$html.='<td class="text-right">'.$value['ndebitamt'].'</td>';
		$html.='<td class="text-right">'.$value['nbalamt'].'</td>';
		$html.='</tr>';	
	}
  ...
  public function pdf_save_page() {
	...
	protected function getList() {
		...
		$data['current_url'] = $this->url->link('administration/customer_report', 'token=' . $this->session->data['token'], true);
		$data['searchcustomer'] = $this->url->link('administration/customer_report/search', 'token=' . $this->session->data['token'], true);
