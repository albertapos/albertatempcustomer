...
class ControllerAdministrationPaidOut extends Controller {
	...
	protected function getList() {
		...
		$data['current_url'] = $this->url->link('administration/paid_out', 'token=' . $this->session->data['token'], true);
		$data['searchpaid_out'] = $this->url->link('administration/paid_out/search', 'token=' . $this->session->data['token'], true);
	...
	protected function validateEditList() {
  	...
  	public function search(){
		...
		if(isset($this->request->get['term']) && !empty($this->request->get['term'])){
			...
			$datas = $this->model_api_paid_out->getPaidOutSearch($this->request->get['term']);

			foreach ($datas as $key => $value) {
				$temp = array();
				$temp['ipaidoutid'] = $value['ipaidoutid'];
				$temp['vpaidoutname'] = $value['vpaidoutname'];
				$return[] = $temp;
			}
		}
