...
class ControllerAdministrationProductListingReport extends Controller {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
      ...
      exit;
        ...
        $data['check_password_url'] = $this->url->link('administration/product_listing_report/check_password', 'token=' . $this->session->data['token'] , true);
    ...
    public function check_password(){
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            if(isset($this->request->post['password']) && $this->request->post['password'] != ""){
                if($this->request->post['password'] === "Alberta@POS"){
                    echo "true";
                }
                else{
                    echo "false";
