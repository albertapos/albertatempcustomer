...
class ControllerAdministrationWasteDetail extends Controller {
	...
	protected function getForm() {
		...
		$data['display_items_search'] = $this->url->link('administration/waste_detail/display_items_search', 'token=' . $this->session->data['token'], true);
	...
	protected function validateEditList() {
  	...
  	public function add_items() {
		...
		if(count($this->request->post['checkbox_itemsort1']) > 0){
			$right_items_arr = $this->model_administration_waste_detail->getRightItems($this->request->post['checkbox_itemsort1']);
	...
	public function display_items_search() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if(isset($post_arr['search_val']) && isset($post_arr['search_by']) && isset($post_arr['right_items'])){
					...
					$json['items'] = $this->model_administration_waste_detail->getSearchItems($post_arr);
			}
		}
