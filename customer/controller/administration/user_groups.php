...
class ControllerAdministrationUserGroups extends Controller {
	...
	protected function getList() {
		...
		$data['current_url'] = $this->url->link('administration/user_groups', 'token=' . $this->session->data['token'], true);
		$data['searchuser_groups'] = $this->url->link('administration/user_groups/search', 'token=' . $this->session->data['token'], true);
	...
	public function search(){
		...
		if(isset($this->request->get['term']) && !empty($this->request->get['term'])){
			...
			$datas = $this->model_api_user_groups->getUserGroupsSearch($this->request->get['term']);

			foreach ($datas as $key => $value) {
				$temp = array();
				$temp['ipermissiongroupid'] = $value['ipermissiongroupid'];
				$temp['vgroupname'] = $value['vgroupname'];
				$return[] = $temp;
			}
		}
