...
class ControllerAdministrationInventoryOnHandReport extends Controller {
  ...
  protected function getList_test() {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
      ...
      $print_page = $this->url->link('administration/inventory_on_hand_report/print_page', 'token=' . $this->session->data['token'] . $url, true);

      $html = file_get_contents($print_page);
      ...
      fwrite ($pdf1,$decoded_html);
      fclose ($pdf1);
      
      //$testing = $this->dompdf->loadHtml($html);
      //print_r($testing);
      exit;
