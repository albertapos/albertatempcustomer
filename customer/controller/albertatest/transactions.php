<?php
class ControllerAdministrationTransactions extends Controller {
	private $error = array();

	public function index() {

		$this->load->language('administration/transactions');

		$this->document->setTitle($this->language->get('heading_title'));

    	$this->load->model('administration/transactions');
  ...
  public function sales_view($id=0) {
	...
	$data['heading_title'] = 'Transactions';
		
    $salesid = ($this->request->get['salesid'])?$this->request->get['salesid']:$id;
	
	$print_page = $this->url->link('administration/transactions/print_page', 'token=' . $this->session->data['token'] .'&salesid='.$salesid, true);
	...
	$sales_header= $this->model_api_transactions->getSalesById($salesid);

	$sales_detail= $this->model_api_transactions->getSalesPerview($salesid);
	
	$sales_tender= $this->model_api_transactions->getSalesByTender($salesid);
	
	$html='';

	if(count($store_info) > 0 && count($sales_header) > 0 && count($sales_detail) > 0 && count($sales_tender) > 0){
	  ...
	  $html.='</td>
		...
		$html .= '<h3 class="text-center">Sorry no data found!</h3>';
	}
  ...
  <div class="container-fluid">
	...
	$myfile = fopen( DIR_TEMPLATE."/administration/print_transactions.tpl", "w");
	fwrite($myfile,$file);
	fclose($myfile);
  ...
  public function pdf_save_page() {
	...
	protected function getList() {
		...
		//if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			//$report_datas = $this->model_api_transactions->getSalesReport($this->request->post);
			$sales_total = $this->model_api_transactions->getSalesReportTotal($filter_data);
			
			$report_datas = $this->model_api_transactions->getSalesReport($filter_data);
			...
			$pagination->url = $this->url->link('administration/transactions', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('administration/transactions', 'token=' . $this->session->data['token'], true)
		);

		$data['reportdata'] = $this->url->link('administration/transactions/reportdata', 'token=' . $this->session->data['token'] . $url, true);
		$data['print_page'] = $this->url->link('administration/transactions/print_page', 'token=' . $this->session->data['token'] . $url, true);
		$data['pdf_save_page'] = $this->url->link('administration/transactions/pdf_save_page', 'token=' . $this->session->data['token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('administration/transactions_list', $data));
	}
  ...
  public function reportdata(){
	...
	public function printpage(){
		$this->response->setOutput($this->load->view('administration/print_transactions'));	
	}
}
