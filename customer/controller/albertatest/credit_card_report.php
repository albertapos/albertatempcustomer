<?php
class ControllerAlbertatestCreditCardReport extends Controller {
	private $error = array();

	public function index() {
    $this->load->language('albertatest/credit_card_report');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('albertatest/credit_card_report');
  ...
  public function print_receipt() {
    ...
    $data['receipt'] = $this->model_albertatest_credit_card_report->getReceiptData($id,$by);

    $salesid = $data['receipt']['isalesid'];

    $store_info= $this->model_api_store->getStore();
    $sales_header= $this->model_albertatest_credit_card_report->getSalesById($salesid);
    ...
    $sales_detail= $this->model_albertatest_credit_card_report->getSalesPerview($salesid);

    $sales_tender= $this->model_albertatest_credit_card_report->getSalesByTender($salesid);

    $sales_customer= $this->model_albertatest_credit_card_report->getSalesByCustomer($sales_header['icustomerid']);
    ...
    $this->response->setOutput($this->load->view('albertatest/print_credit_card_report_reciept', $data));
  }
  ...
  public function pdf_save_page() {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
      ...
      $data['reports'] = $this->model_albertatest_credit_card_report->getCreditCardReport($this->request->post);
      ...
      $this->session->data['p_end_date'] = $data['p_end_date'];
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('albertatest/credit_card_report', 'token=' . $this->session->data['token'] . $url, true)
		);

    $data['report_ajax_data'] = $this->url->link('albertatest/credit_card_report/report_ajax_data', 'token=' . $this->session->data['token'] . $url, true);
    $data['reportdata'] = $this->url->link('albertatest/credit_card_report/reportdata', 'token=' . $this->session->data['token'] . $url, true);
    $data['print_page'] = $this->url->link('albertatest/credit_card_report/print_page', 'token=' . $this->session->data['token'] . $url, true);
    $data['pdf_save_page'] = $this->url->link('albertatest/credit_card_report/pdf_save_page', 'token=' . $this->session->data['token'] . $url, true);
    $data['csv_export'] = $this->url->link('albertatest/credit_card_report/csv_export', 'token=' . $this->session->data['token'] . $url, true);

    $data['print_receipt'] = $this->url->link('albertatest/credit_card_report/print_receipt', 'token=' . $this->session->data['token'] . $url, true);
    ...
    $data['store_name'] = $this->session->data['storename'];
		...
		$this->response->setOutput($this->load->view('albertatest/credit_card_report_list', $data));
	}
  ...
  public function report_ajax_data() {
    ...
    if ($this->request->server['REQUEST_METHOD'] == 'POST') {
      ...
      if(!empty($temp_arr['report_pull_by'])){
        $data = $this->model_albertatest_credit_card_report->ajaxDataCreditCardReport($temp_arr);
      }else{
        $data = array();
      }
