...
class ModelApiDepartment extends Model {
    ...
    public function addDepartment($datas = array()) {
        ...
        if(isset($datas) && count($datas) > 0){
            foreach ($datas as $key => $value) {
               ...
               try {
                    $sql = "INSERT INTO mst_department SET vdepartmentname = '" . $this->db->escape($value['vdepartmentname']) . "',`vdescription` = '" . $this->db->escape($value['vdescription']) . "',";
    ...
    public function addDepartmentByName($depname) {
        ...
        try {
            $sql = "INSERT INTO mst_department SET vdepartmentname = '" . $this->db->escape($depname) . "',isequence =0 ,estatus = 'Active',SID = '" . (int)($this->session->data['sid'])."'";
        ...
        if(isset($datas) && count($datas) > 0){
            foreach ($datas as $key => $value) {
              ...
              try {
                    ...
                    $sql = "UPDATE mst_department SET vdepartmentname = '" . $this->db->escape($value['vdepartmentname']) . "',`vdescription` = '" . $this->db->escape($value['vdescription']) . "',";
