<?php
class ModelApiPhysicalInventory extends Model {
                        ...
                        if($parent_item['visinventory'] == 'Yes'){
                            if($parent_item['isparentchild'] == 1){
                                if($data['vtype'] == 'Waste'){
                                    ...
                                    $update_sql = "UPDATE mst_item SET  iqtyonhand = '" . $this->db->escape($item['itotalunit']) . "' WHERE iitemid='" . (int)$parent_item['parentmasterid'] . "'";
                                    $this->db2->query($update_sql);
                                ...
                                if($data['vtype'] == 'Waste'){
                                    ...
                                    $x_general->old_item_values = $old_item_values;
                                    try{
                                    // echo "INSERT INTO trn_webadmin_history SET  itemid = '" . $parent_item['iitemid'] . "',userid = '" . $this->session->data['user_id'] . "',barcode = '" . $this->db->escape($parent_item['vbarcode']) . "', type = 'QOH', oldamount = '" . $this->db->escape($parent_item['iqtyonhand']) . "', newamount = '". ($this->db->escape($parent_item['iqtyonhand']) - $this->db->escape($item['ndebitqty'])) ."', source = 'Adjustment', historydatetime = NOW(),SID = '" . (int)($this->session->data['sid'])."'";exit;
                                    $this->db2->query("INSERT INTO trn_webadmin_history SET  itemid = '" . $parent_item['iitemid'] . "',userid = '" . $this->session->data['user_id'] . "',barcode = '" . $this->db->escape($parent_item['vbarcode']) . "', type = 'QOH', oldamount = '" . $this->db->escape($parent_item['iqtyonhand']) . "', newamount = '". ($this->db->escape($parent_item['iqtyonhand']) - $this->db->escape($item['ndebitqty'])) ."', source = 'Adjustment', historydatetime = NOW(),SID = '" . (int)($this->session->data['sid'])."'");
                                    }
                                    catch (Exception $e) {
                                        $this->log->write($e);
                                    ...
                                    $this->db2->query("UPDATE mst_item SET  iqtyonhand = iqtyonhand-'" . $this->db->escape($item['ndebitqty']) . "' WHERE iitemid='" . (int)$this->db->escape($item['vitemid']) . "'");
