            ...
            $dreceiveddate = $dreceiveddate->format('Y-m-d').' '.$currenttime;
               ...
               try {
                    ...
                    if(isset($data['items']) && count($data['items']) > 0){
						...
						$riptotalamount =0;
                        $data_row .= json_encode($data['items']['selected_items']);
						foreach ($data['items'] as $key => $item) {
...
// fwrite($myfile,$data_row);
                    ...
                    if(count($current_item) > 0 && $data['receive_po'] == 'receivetostore'){
                        if($current_item['isparentchild'] == 1){
                            ...
                            //============================================= New query to insert last cost ======================================================================
                                ...
                                $this->db2->query("UPDATE mst_item SET dcostprice = '" . $new_dcostprice . "',npack = '" . $npack . "',iqtyonhand = '0',nunitcost = '" . $new_nunitcost . "',last_costprice='" . $current_item['nunitcost'] . "',new_costprice= '". $new_costprice ."' WHERE iitemid='" . (int)$this->db->escape($item['vitemid']) . "' ");                               $this->db2->query("UPDATE mst_item SET dcostprice = '" . $new_dcostprice . "',npack = '" . $npack . "',iqtyonhand = '0',nunitcost = '" . $new_nunitcost . "',last_costprice='" . $current_item['nunitcost'] . "',new_costprice= '". $new_costprice ."' WHERE iitemid='" . (int)$this->db->escape($item['vitemid']) . "' ");
    ...
    public function getSearchItemHistoryAll($search_item,$ivendorid,$pre_items_id) {
        ...
        if(count($pre_items_id) > 0){
            $pre_items_id = implode(',', $pre_items_id);
            $query = $this->db2->query("SELECT DISTINCT mi.iitemid,mi.vitemcode,mi.vitemname,mi.vbarcode,mi.vsize,mi.iqtyonhand,mi.dcostprice,mi.dunitprice,mi.npack,CASE WHEN mi.npack = 1 or (mi.npack is null)   then mi.iqtyonhand else (Concat(cast(((mi.iqtyonhand div mi.npack )) as signed), '  (', Mod(mi.iqtyonhand,mi.npack) ,')') ) end as QOH FROM mst_item as mi  LEFT JOIN mst_itemalias as mia ON(mi.vitemcode=mia.vitemcode) WHERE (mi.vitemname LIKE  '%" .$this->db->escape($search_item). "%' OR mi.vbarcode LIKE  '%" .$this->db->escape($search_item). "%' OR mi.vsize LIKE  '%" .$this->db->escape($search_item). "%' OR mia.valiassku LIKE  '%" .$this->db->escape($search_item). "%' OR mi.dcostprice LIKE  '%" .$this->db->escape($search_item). "%' OR mi.dunitprice LIKE  '%" .$this->db->escape($search_item). "%') AND mi.iitemid NOT IN($pre_items_id) AND mi.estatus='Active'");
        }else{
            $query = $this->db2->query("SELECT DISTINCT mi.iitemid,mi.vitemcode,mi.vitemname,mi.vbarcode,mi.vsize,mi.iqtyonhand,mi.dcostprice,mi.dunitprice,mi.npack,CASE WHEN mi.npack = 1 or (mi.npack is null)   then mi.iqtyonhand else (Concat(cast(((mi.iqtyonhand div mi.npack )) as signed), '  (', Mod(mi.iqtyonhand,mi.npack) ,')') ) end as QOH FROM mst_item as mi  LEFT JOIN mst_itemalias as mia ON(mi.vitemcode=mia.vitemcode) WHERE (mi.vitemname LIKE  '%" .$this->db->escape($search_item). "%' OR mi.vbarcode LIKE  '%" .$this->db->escape($search_item). "%' OR mi.vsize LIKE  '%" .$this->db->escape($search_item).  "%' OR mia.valiassku LIKE  '%" .$this->db->escape($search_item). "%' OR mi.dcostprice LIKE  '%" .$this->db->escape($search_item). "%' OR mi.dunitprice LIKE  '%" .$this->db->escape($search_item). "%') AND mi.estatus='Active'");
        }
