        ...
        if(isset($data) && count($data) > 0){
              ...
              try {
                    ...
                    foreach($data as $value){
                        ...
                        // echo "<pre>";print_r($current_item);exit;
                        
                        /*if($current_item['dunitprice'] != $value['dunitprice'] && $value['dunitprice'] != '')
                        {
                        ...
                        {
                            $this->db2->query("UPDATE mst_item SET nunitcost = '" . $this->db->escape($value['nunitcost']) . "' WHERE iitemid = '" . (int)$value['iitemid'] . "'");
                        }
                        if($current_item['vdepcode'] != $value['vdepartmentname'] && $value['vdepartmentname'] != '')
                        {
                            $this->db2->query("UPDATE mst_item SET vdepcode = '" . $this->db->escape($value['vdepartmentname']) . "' WHERE iitemid = '" . (int)$value['iitemid'] . "'");
                        }
                        if($current_item['vcategorycode'] != $value['vcategoryname'] && $value['vcategoryname'] != '')
                        {
                            $this->db2->query("UPDATE mst_item SET vcategorycode = '" . $this->db->escape($value['vcategoryname']) . "' WHERE iitemid = '" . (int)$value['iitemid'] . "'");
                        }if($current_item['vcategorycode'] != $value['vcategoryname'] && $value['vcategoryname'] != '')
                        {
                            $this->db2->query("UPDATE mst_item SET vcategorycode = '" . $this->db->escape($value['vcategoryname']) . "' WHERE iitemid = '" . (int)$value['iitemid'] . "'");
                        }
                        ...
                        if((($current_item['dcostprice'] != $value['dcostprice']) && ($current_item['dcostprice'] != '0.0000') && $current_item['isparentchild'] !=1) && (($current_item['dunitprice'] != $value['dunitprice']) && ($current_item['dunitprice'] != '0.00'))){
                            ...
                            // Changed on Vijay's request: Unit price changes even if the previous price is 0.00
