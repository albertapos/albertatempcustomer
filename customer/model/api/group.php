...
class ModelApiGroup extends Model {
    ...
    public function getGroupByName($group) {
        ...
        $sql = "SELECT * FROM itemgroup WHERE vitemgroupname='".$this->db->escape($group)."'";
    ...
    public function addGroupByName($group, $sku) {

        $success =array();
        $error =array();

       try {
            $this->db2->query("INSERT INTO itemgroup SET  `vitemgroupname` = '" . $this->db->escape($group) . "',`etransferstatus` = '',SID = '" . (int)($this->session->data['sid'])."'");
            $iitemgroupid = $this->db2->getLastId();
            
            $this->db2->query("INSERT INTO itemgroupdetail SET  `iitemgroupid` = '" . (int)$iitemgroupid . "',`vsku` = '" . $this->db->escape($sku) . "',`isequence` = '',`vtype` = '',SID = '" . (int)($this->session->data['sid'])."',`Id` = '". (int)$iitemgroupid . "'");
        }
        catch (MySQLDuplicateKeyException $e) {
            // duplicate entry exception
           $error['error'] = $e->getMessage(); 
