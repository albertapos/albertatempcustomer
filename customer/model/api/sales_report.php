...
class ModelApiSalesReport extends Model {
	...
	public function getSalesReport($data) {
		...
		$sql = "SELECT (a.dtrandate) as dtrandate,vterminalid,concat('TRN',isalesid) as  trnsalesid,isalesid,nnettotal,nchange,ntaxtotal,vtendertype,vtrntype FROM trn_sales a WHERE date_format(a.dtrandate,'%Y-%m-%d') >= '".$data['start_date']."' and date_format(a.dtrandate,'%Y-%m-%d') <= '".$data['end_date']."' AND a.vtrntype='Transaction' ";
