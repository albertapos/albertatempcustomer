...
class ModelAdministrationAdjustmentDetail extends Model {
	...
	public function getRightItems($data = array()) {
		...
		if(count($data) > 0){
			...
			$query = $this->db2->query("SELECT iitemid,vbarcode,vitemname,npack,nunitcost,iqtyonhand FROM mst_item WHERE iitemid IN($item_ids) AND estatus='Active'");
			$return = $query->rows;
		}else{
	...
	public function getSearchItems($data = array()) {
    	...
    	if(isset($data['search_by']) && $data['search_by'] == 'vbarcode'){
    		if($item_ids != ''){
    			$query = $this->db2->query("SELECT iitemid,vbarcode,vitemname FROM mst_item FORCE INDEX (idx_item_vbarcode) WHERE vbarcode LIKE  '%" .$this->db->escape($data['search_val']). "%' AND estatus='Active' AND iitemid NOT IN($item_ids)");
    		}else{
    			$query = $this->db2->query("SELECT iitemid,vbarcode,vitemname FROM mst_item FORCE INDEX (idx_item_vbarcode) WHERE vbarcode LIKE  '%" .$this->db->escape($data['search_val']). "%' AND estatus='Active'");
    		}
    		
    	}else{
    		if($item_ids != ''){
    			$query = $this->db2->query("SELECT iitemid,vbarcode,vitemname FROM mst_item FORCE INDEX (idx_item_vitemname) WHERE vitemname LIKE  '%" .$this->db->escape($data['search_val']). "%' AND estatus='Active' AND iitemid NOT IN($item_ids)");
    		}else{
    			$query = $this->db2->query("SELECT iitemid,vbarcode,vitemname FROM mst_item FORCE INDEX (idx_item_vitemname) WHERE vitemname LIKE  '%" .$this->db->escape($data['search_val']). "%' AND estatus='Active'");
    		}
    	}
